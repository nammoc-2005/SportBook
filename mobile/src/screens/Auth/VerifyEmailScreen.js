...
const VerifyEmailScreen = ({ navigation }) => {
  ...
  const sendOTP = async () => {
    setSending(true);
    try {
      await api.post('/auth/send-email-otp', { email: userInfo.email });
      setCountdown(60);
    } catch (e) {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/auth/verify-email-otp', { email: userInfo.email, otp: otpCode });
      if (res.data.success) {
        Alert.alert('Thành công', 'Đã xác thực Email!');
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            ...
            <Text style={{ fontWeight: 'bold', color: '#F8FAFC' }}>{userInfo?.email}</Text>
          </Text>
        </View>
